...
########################################
    ...
    bas = st.date_input("Poliçe Başlangıcı")
    bit = st.date_input("Poliçe Bitişi")
    months = (bit.year-bas.year)*12+(bit.month-bas.month)+(1 if bit.day>=15 else 0)
    st.markdown(f"**Süre:** {months} ay")
    koas = st.selectbox("Koasürans", list(koasurans_indirimi.keys()), key="ko_c")
    mua  = st.selectbox("Muafiyet (%)", list(muafiyet_indirimi.keys()), key="mu_c")
    ccy  = st.selectbox("Para Birimi", ["TRY","USD","EUR"], key="cur_c")
    kur  = fx_input(ccy, "car")
    st.markdown("---")
    car_bed = st.number_input("Proje Bedeli (CAR)", min_value=0, step=1_000_000)
    cpm_bed = st.number_input("İnşaat Makineleri (CPM)", min_value=0, step=1_000_000)
    cpe_bed = st.number_input("Şantiye Tesisleri (CPE)", min_value=0, step=1_000_000)
    if st.button("Hesapla", key="car_calc"):
        def sure_carp(m):
            return sure_carpani_tablosu.get(m, sure_carpani_tablosu[36]+0.03*(m-36))
        k_ind = koasurans_indirimi[koas]
        m_ind = muafiyet_indirimi[mua]
        sc = sure_carp(months)
        def car_prim(bed):
            tl = bed*kur
            oran = car_tarife_oranlari[risk][deprem_bolgesi-1]/1000*sc
            if tl<850_000_000:
                return bed*oran*(1-k_ind)*(1-m_ind)
            return 850_000_000*oran*(1-k_ind)*(1-m_ind)/kur
        def cpm_prim(bed):
            tl=bed*kur; oran=0.002 if tl<850_000_000 else (0.002*850_000_000)/tl
            return bed*oran
        def cpe_prim(bed):
            tl=bed*kur; oran=0.0012495
            return bed*oran if tl<850_000_000 else 850_000_000*oran/kur
        p_car, p_cpm, p_cpe = car_prim(car_bed), cpm_prim(cpm_bed), cpe_prim(cpe_bed)
        st.success(f"Toplam Minimum Prim: {(p_car+p_cpm+p_cpe):,.2f} TL")
